...
parasails.registerComponent("ajaxForm", {
  ...
  `,
            ...
            for (let fieldName in this.formRules) {
                for (let ruleName in this.formRules[fieldName]) {
                    if (_.contains(SUPPORTED_RULES, ruleName)) {
                        ...
                        let ruleIdx = -1;
                        if (_.indexOf(kebabRules, ruleName) === -1) {
                            if (_.indexOf(lowerCaseRules, ruleName.toLowerCase()) === -1) {
                                ruleIdx = -1;
                            } else {
                                ruleIdx = _.indexOf(lowerCaseRules, ruleName.toLowerCase());
                            }
                        } else {
                            ruleIdx = _.indexOf(kebabRules, ruleName);
                        }
    ...
    "methods": {
        ...
        "keydownMetaEnter": async function keydownMetaEnter() {
            await this._submit();
        },
        ...
        //  ╩  ╩╚═╩ ╚╝ ╩ ╩ ╩ ╚═╝  ╩ ╩╚═╝ ╩ ╩ ╩╚═╝═╩╝╚═╝
        // eslint-disable-next-line complexity
        "_submit": async function _submit() {
